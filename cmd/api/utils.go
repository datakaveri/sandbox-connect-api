...
func generateNotebookURL(baseURL, namespace, notebookName string, imageName *string, disableInit bool, startupFileName string) string {
	if baseURL == "" {
		return ""
	}
	baseURL = strings.TrimSuffix(baseURL, "/")

	// With init containers disabled there is no startup notebook on the PVC, so
	// send users to the JupyterLab auto workspace instead of a missing file.
	if disableInit {
	...
	// Without a startup notebook configured, open the file browser rather than a
	// file that may not exist.
	if fileName == "" {
		return fmt.Sprintf("%s/notebook/%s/%s/lab/tree", baseURL, namespace, notebookName)
	}
