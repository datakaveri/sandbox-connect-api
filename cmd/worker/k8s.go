...
// DeleteNotebook deletes a Notebook custom resource in the specified namespace
func DeleteNotebook(k8sClient *k8s.K8sClient, namespace, notebookName string) error {
	...
	// Check if the notebook exists before trying to delete it
	...
	if err != nil {
		...
		return nil // Return nil as this is not a critical error - resource is already gone
	}

	// Delete the notebook with a background deletion policy
...
func DeletePod(k8sClient *k8s.K8sClient, namespace, podName string) error {
	...
	// Check if the pod exists before trying to delete it
	...
	// Delete the pod with a background deletion policy
	deletePolicy := metav1.DeletePropagationBackground
	deleteOptions := metav1.DeleteOptions{
		PropagationPolicy: &deletePolicy,
	}
...
// DeletePVC deletes a PersistentVolumeClaim in the specified namespace
func DeletePVC(k8sClient *k8s.K8sClient, namespace, pvcName string) error {
	...
	// Check if the PVC exists before trying to delete it
	...
	// Delete the PVC with a background deletion policy
	deletePolicy := metav1.DeletePropagationBackground
	deleteOptions := metav1.DeleteOptions{
		PropagationPolicy: &deletePolicy,
	}
