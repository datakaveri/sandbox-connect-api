...
// FetchNotebooksForCleanup fetches notebooks that need to be cleaned up
// This function looks for notebooks that have the 'cleanup-requested' event in their events array
// and marks them as being processed for cleanup
func FetchNotebooksForCleanup(pg *db.PgPool, ctx context.Context) ([]Notebook, error) {
	tx, err := pg.Pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return nil, err
	}
	...
	// Query to find notebooks that have 'cleanup-requested' in their events array
	// and have not been picked up for cleanup yet (no 'cleanup-started' event)
	query := `
		SELECT id, name, namespace, storage_size, pvc_name, cpu_request, cpu_limit, 
		       memory_request, memory_limit, gpu_type, gpu_count, template_name
		FROM notebooks
		WHERE 'cleanup-requested' = ANY(events)
		AND NOT 'cleanup-started' = ANY(events)
		FOR UPDATE SKIP LOCKED
		LIMIT 1
	`
	rows, err := tx.Query(ctx, query)
	...
	// Mark the notebook as being processed for cleanup
	updateQuery := `
	UPDATE notebooks
	SET picked_at = NOW()
	WHERE id = $1
        `
	_, err = tx.Exec(ctx, updateQuery, notebooks[0].ID)
	if err != nil {
		return nil, err
	}
